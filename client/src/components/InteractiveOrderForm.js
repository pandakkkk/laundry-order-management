...
const InteractiveOrderForm = memo(({ onSubmit, onCancel }) => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      if (response.success && response.data && response.data.length > 0) {
        ...
        // API returned empty or invalid - use static catalog (includes all categories)
        setProducts(STATIC_PRODUCTS);
      }
    } catch (error) {
      console.error('Error fetching products, using static data:', error);
      // Fall back to static products when API fails
      setProducts(STATIC_PRODUCTS);
    } finally {
  ...
  const handleSubmit = (e) => {
    ...
    // B2B order if any item has B2B product
    const isB2BOrder = cart.some(item => item.productId && item.productId.startsWith('b2b_'));

    const { ticketNumber, orderNumber, ...cleanCustomerInfo } = customerInfo;

    const orderData = {
      ...cleanCustomerInfo,
      // ticketNumber and orderNumber will be auto-generated by server if empty or stale
      orderDate: new Date().toISOString(),
      expectedDelivery: customerInfo.expectedDelivery ? new Date(customerInfo.expectedDelivery).toISOString() : undefined,
      pickupSchedule: customerInfo.pickupSchedule ? new Date(customerInfo.pickupSchedule).toISOString() : undefined,
      items: cart.map(item => ({
      ...
      orderType: isB2BOrder ? 'b2b' : 'retail',
  ...
  // Get products filtered by category and search query
  const categoryProducts = products.filter(product => {
    const matchesCategory = product.category === selectedCategory;
    if (!productSearchQuery.trim()) return matchesCategory;
      ...
      <div className="pos-main-content">
                  ...
                  <div className="cart-item-notes">
                    {expandedNotes[item.id] || item.notes ? (
                      <input
                        ...
                        autoFocus={expandedNotes[item.id] && !item.notes}
                        onBlur={() => {
                          if (!item.notes) {
        ...
        <div className="pos-products">
          ...
          <div className="product-search">
            ...
            />
            {productSearchQuery && (
              <button className="btn-icon" onClick={() => setProductSearchQuery('')} title="Clear search">✕</button>
            )}
          </div>
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input
                    ...
                    id="pickupSchedule"
                    name="pickupSchedule"
                    value={customerInfo.pickupSchedule}
                    onChange={handleCustomerInfoChange}
                  />
                </div>
                <div className="form-group">
